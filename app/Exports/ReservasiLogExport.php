...
class ReservasiLogExport implements FromQuery, WithHeadings,WithStyles,WithColumnWidths,WithMapping,ShouldAutoSize,WithCustomStartCell
{
    protected $reservasilog;

    public function query()
    {
        $this->reservasilog = ReservasiLog::query()->get();
        return ReservasiLog::query();
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // tittle
        $sheet->mergeCells("B4:L4")->setCellValue("B4","Log Reservasi");
        $sheet->mergeCells("B5:L5")->setCellValue("B5","Aston Bogor");

        // header
        $sheet->getStyle('B4:L4')->applyFromArray(array(
            'borders' => array(
                'allBorders' => array(
        ...
        $sheet->getStyle('B5:L5')->applyFromArray(array(
            'borders' => array(
                'allBorders' => array(
                    'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                    'color' => ['argb' => '00000'],
        ...
        for ($i=0; $i < count($this->reservasilog) ; $i++) {
